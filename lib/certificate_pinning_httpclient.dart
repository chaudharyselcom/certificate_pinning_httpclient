...
class _CertificatePinningService {
  ...
  static Future<List<Uint8List>?> _getHostCertificates(Uri url) async {
    if (_hostCertificates[url.host] == null) {
      try {
        final arguments = <String, String>{"url": url.toString()};
  ...
  static Future<List<Uint8List>> _hostPinCertificates(
    ...
    for (final cert in hostCertificates) {
      final Uint8List serverSpkiSha256Digest = Uint8List.fromList(
        _spkiSha256Digest(cert).bytes,
      );
  ...
  static Future<SecurityContext> _pinnedSecurityContext(
    ...
    for (final pinCert in pinCerts) {
      ...
      final Uint8List pemCertificatesBytes = const AsciiEncoder().convert(
        pemCertificate,
      );
  ...
  static Future<bool> getJailbroken() async {
    final arguments = <String, String>{};
    bool? jailbroken = await _channel.invokeMethod(
      'jailbroken',
    );
  ...
  static Future<bool> getDeveloperMode() async {
    final arguments = <String, String>{};
    bool? developerMode = await _channel.invokeMethod(
     'jailbroken',
       arguments,
     ); //
...
class CertificatePinningHttpClient implements HttpClient {
  // logging tag
  static const String _tag = "CertificatePinningHttpClient_selcome";
  ...
  static Future<bool> isJailbroken() async {
    // construct a new http client
    bool jailbroken = await _CertificatePinningService.getJailbroken();
    return jailbroken ?? false;
  }

  static Future<bool> isDeveloperMode() async {
    // construct a new http client
    bool developerMode = await _CertificatePinningService.getDeveloperMode();
